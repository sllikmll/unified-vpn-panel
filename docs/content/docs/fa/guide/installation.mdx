...
docker run -d \
  ...
  --name 3x-ui \
  ghcr.io/sllikmll/unified-vpn-panel:latest
```
...
[صفحهٔ releases](https://github.com/sllikmll/unified-vpn-panel/releases) دانلود کنید، آن را
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/sllikmll/unified-vpn-panel/main/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/sllikmll/unified-vpn-panel/tree/main/deploy) در مخزن
