...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: Run an ephemeral SSH preflight against a candidate node without saving
        credentials. Supports write-only password or private-key auth, strict
        host-key verification by known_hosts or fingerprint pin, and explicit
        insecure opt-in. Returns OS, arch, hostname, privilege capability,
        systemd/Docker, free disk, occupied ports, and provisioning readiness.
      url: '#run-an-ephemeral-ssh-preflight-against-a-candidate-node-without-saving-credentials-supports-write-only-password-or-private-key-auth-strict-host-key-verification-by-known_hosts-or-fingerprint-pin-and-explicit-insecure-opt-in-returns-os-arch-hostname-privilege-capability-systemddocker-free-disk-occupied-ports-and-provisioning-readiness'
    - depth: 2
  ...
  structuredData:
    headings:
      ...
      - content: Run an ephemeral SSH preflight against a candidate node without saving
          credentials. Supports write-only password or private-key auth, strict
          host-key verification by known_hosts or fingerprint pin, and explicit
          insecure opt-in. Returns OS, arch, hostname, privilege capability,
          systemd/Docker, free disk, occupied ports, and provisioning readiness.
        id: run-an-ephemeral-ssh-preflight-against-a-candidate-node-without-saving-credentials-supports-write-only-password-or-private-key-auth-strict-host-key-verification-by-known_hosts-or-fingerprint-pin-and-explicit-insecure-opt-in-returns-os-arch-hostname-privilege-capability-systemddocker-free-disk-occupied-ports-and-provisioning-readiness
