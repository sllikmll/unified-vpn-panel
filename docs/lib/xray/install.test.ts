...
describe('buildScriptCommand', () => {
  ...
  it('pins a specific version by passing the tag to main install.sh', () => {
    const cmd = buildScriptCommand({ ...base, version: 'v0.0.1' });
    expect(cmd).toBe(
...
describe('buildDockerRun', () => {
  it('reflects fail2ban, port, and base path options', () => {
    ...
    expect(cmd).toContain('ghcr.io/sllikmll/unified-vpn-panel:latest');
...
describe('buildDockerCompose', () => {
  it('produces valid-looking compose with the image and volumes', () => {
    ...
    expect(yaml).toContain('image: ghcr.io/sllikmll/unified-vpn-panel:latest');
