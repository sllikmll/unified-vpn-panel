...
export type EndpointClientState = string;
export type EndpointStatus = string;
export type ManagedDriverProvider = unknown;
export type ManagedEndpointSource = string;
export type ManagedProtocol = string;
export type ManagedSecretKeySource = unknown;
...
export interface ManagedEndpointCapabilities {
  runtimeKinds: ManagedEndpointCapability[];
}

export interface ManagedEndpointCapability {
  clientCrud: boolean;
  detect: boolean;
...
export interface ManagedEndpointView {
  ...
  tag: string;
  traffic: ManagedTrafficView;
}

export interface ManagedHealthView {
  checkedAt: number;
  message: string;
  status: EndpointStatus;
}
...
export interface ManagedTrafficView {
  down: number;
  up: number;
}
...
export interface Node {
  ...
  appStatsMem: number;
  appStatsThreads: number;
  appStatsUptime: number;
  basePath: string;
  clientCount: number;
  configDirty: boolean;
  ...
  name: string;
  netDown: number;
  netTrafficRecv: number;
  netTrafficSent: number;
  netUp: number;
  onlineCount: number;
  outboundTag: string;
  panelVersion: string;
